...
Upstash Redis is now provisioned on its free plan, with automatic upgrades disabled. Production receives the worker's authenticated aggregate reports automatically. Real integration checks verified Redis access, unauthorized-write rejection, safe duplicate retries, rejection of an older report, and successive fresh automatic reports. Desktop and 390px mobile checks showed the actual simulated accounts and live feed state. See the [dated relay activation evidence](https://tajari.vercel.app/evidence/continuous-paper-relay.json). No paid plan was purchased.

Publishing was enabled by a deliberate flat-state stop, a journaled configuration change and a restart of the same immutable worker release. The STOP decision was archived. The original registration, protocol, accounts and outage exclusions remain intact. At verification both accounts were still flat at simulated $50,000, with zero orders, fills or complete opening opportunities. The shared dashboard connection does not make the Mac worker independent of sleep, power or network loss.
...
The local read endpoint listens only on `127.0.0.1:8022/status` and requires the private `TAJARI_PAPER_LOCAL_TOKEN`. Local development can use `TAJARI_PAPER_SERVICE_URL` and that token. Production instead uses the Upstash REST credentials and pinned `TAJARI_PAPER_RUN_ID`. The worker publishes to `TAJARI_PAPER_STATUS_URL` with a separate private `TAJARI_PAPER_STATUS_TOKEN`; the same publishing token is a production server secret. Browsers receive none of these credentials. Public responses use a field allowlist and show stale status after 150 seconds without a new report. The publisher sends aggregate simulated values, counts and operational state; it excludes licensed prices and private ledgers. Reports remain stored during outages so the last observation stays visible as stale.
