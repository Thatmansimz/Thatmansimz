...
The production partner view now receives authenticated aggregate reports through Upstash Redis on its free plan, with automatic upgrades disabled. The actual worker-to-store-to-browser flow was verified, including rejection of unauthenticated and older reports. See the [relay activation evidence](https://tajari.vercel.app/evidence/continuous-paper-relay.json). The worker remains on the Mac; persistent hosting, off-host recovery and independent alerts remain unfinished. Existing live data access works without a plan change; no new paid service was purchased.
